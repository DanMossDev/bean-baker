...
function setup() {
    ...
    const beans = new PIXI.Sprite(
        loader.resources['beans'].texture
    )
    ...
    beanCount = new PIXI.Text(`Beans: ${beanNum} (${income} per second)`,{fontFamily: "Courier New", fontWeight: "bold", fontSize: 24, fill : 0x42032C, align : 'center'})
...
class Beaner {
    ...
    constructor(name, baseCost, baseIncome, position = 1) {
        ...
        this.stats = new PIXI.Text(`${this.name}: ${this.count}\nIncome: ${this.income}`,{fontFamily: "Courier New", fontWeight: "bold", fontSize: 24, fill : 0x42032C, align : 'center'})
        this.buy = new PIXI.Text(`Buy(${this.cost} beans)`,{fontFamily: "Courier New", fontWeight: "bold", fontSize: 20, fill : 0x42032C, align : 'center'})
        this.button = new PIXI.Sprite(
            loader.resources['button'].texture
        )
...
class BeanParticle {
    ...
    constructor(xPos, yPos) {
        ...
        this.texture = new PIXI.Sprite(
            loader.resources['happy'].texture
        )
        this.texture.x = xPos
        this.texture.y = yPos
